...
public class GetUserAsyncTask extends AsyncTask<Void, Void, Pair<Integer, ResponseEntity<String>>> {

    private String request;
    private ProjectInteractorImpl.OnGetUserListener listener;

    public GetUserAsyncTask(String request, ProjectInteractorImpl.OnGetUserListener listener) {
    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> result) {
        ...
        if (result.first == StatusCode.HTTP_OK) {
            try {
                ...
                listener.onGetUserSuccess(response);
            } catch (JSONException e) {
                listener.onGetUserError(StatusCode.JSON_PARSE_EXCEPTION);
            }
        }
        else {
            listener.onGetUserError(result.first);
        }
    }
