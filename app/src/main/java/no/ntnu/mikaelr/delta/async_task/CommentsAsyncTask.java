    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> result) {
        ...
        if (result.first == StatusCode.HTTP_OK) {
            try {
                ...
                listener.onFinishedLoadingCommentsSuccess(response);
            } catch (JSONException e) {
                listener.onFinishedLoadingCommentsError(StatusCode.JSON_PARSE_EXCEPTION);
            }
        }
        else {
            listener.onFinishedLoadingCommentsError(result.first);
        }
    }
