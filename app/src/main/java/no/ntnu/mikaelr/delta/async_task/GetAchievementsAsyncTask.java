...
public class GetAchievementsAsyncTask extends AsyncTask<Void, Void, Pair<Integer, ResponseEntity<String>>> {

    private String request;
    private ProjectInteractorImpl.OnGetAchievementsListener listener;

    public GetAchievementsAsyncTask(String request, ProjectInteractorImpl.OnGetAchievementsListener listener) {
    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> result) {
        ...
        if (result.first == StatusCode.HTTP_OK) {
            try {
                ...
                listener.onGetAchievementsSuccess(response);
            } catch (JSONException e) {
                listener.onGetAchievementsError(StatusCode.JSON_PARSE_EXCEPTION);
            }
        }
        else {
            listener.onGetAchievementsError(result.first);
        }
    }
