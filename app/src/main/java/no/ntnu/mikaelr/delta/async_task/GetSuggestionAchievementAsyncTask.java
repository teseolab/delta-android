...
public class GetSuggestionAchievementAsyncTask extends AsyncTask<Void, Void, Pair<Integer, ResponseEntity<String>>> {

    private String request;
    private ProjectInteractorImpl.OnGetSuggestionAchievementListener listener;

    public GetSuggestionAchievementAsyncTask(String request, ProjectInteractorImpl.OnGetSuggestionAchievementListener listener) {
    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> result) {
        ...
        if (result.first == StatusCode.HTTP_OK) {
            listener.onGetSuggestionAchievementSuccess(result.second.getBody());
        }
        else {
            listener.onGetSuggestionAchievementError(result.first);
        }
    }
