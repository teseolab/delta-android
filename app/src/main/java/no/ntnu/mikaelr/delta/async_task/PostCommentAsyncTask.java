...
public class PostCommentAsyncTask extends AsyncTask<Void, Void, Pair<Integer, ResponseEntity<String>>> {

    private ProjectInteractorImpl.OnPostCommentListener listener;

    private String request;
    private String body;

    public PostCommentAsyncTask(String request, String body, ProjectInteractorImpl.OnPostCommentListener listener) {
    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> result) {
        ...
        if (result.first == StatusCode.HTTP_OK) {
            try {
                ...
                listener.onPostCommentSuccess(response);
            } catch (JSONException e) {
                listener.onPostCommentError(StatusCode.JSON_PARSE_EXCEPTION);
            }
        }
        else {
            listener.onPostCommentError(result.first);
        }
    }
