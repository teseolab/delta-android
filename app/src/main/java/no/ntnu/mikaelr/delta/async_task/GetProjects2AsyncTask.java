...
public class GetProjects2AsyncTask extends AsyncTask<Void, Void, Pair<Integer, ResponseEntity<String>>> {

    private String request;
    private ProjectInteractorImpl.OnGetProjectsListener listener;

    public GetProjects2AsyncTask(String request, ProjectInteractorImpl.OnGetProjectsListener listener) {
    ...
    protected void onPostExecute(Pair<Integer, ResponseEntity<String>> result) {
        ...
        if (result.first == StatusCode.HTTP_OK) {
            try {
                ...
                listener.onGetProjectsSuccess(response);
            } catch (JSONException e) {
                listener.onGetProjectsError(StatusCode.JSON_PARSE_EXCEPTION);
            }
        }
        else {
            listener.onGetProjectsError(result.first);
        }
    }
